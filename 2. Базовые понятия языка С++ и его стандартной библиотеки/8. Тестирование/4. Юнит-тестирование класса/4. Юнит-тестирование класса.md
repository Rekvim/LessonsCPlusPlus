...
1. Прежде чем писать код, проанализируем условие. Как и в случае с задачей «Выбор наименьшей строки», функциональность программы можно условно разделить на две части:
   
2. Ввод и вывод данных. Этот код разбирает поступающие запросы и выводит результаты.
Работа со словарём синонимов. Этот код реализует логику добавления синонимов, подсчёт их количества, а также проверяет, будут два слова синонимами или нет.

#### Разработка каркаса программы

Сначала разработаем каркас программы, взаимодействующий с пользователем. Для этого подключим библиотеку `<sstream>`. Классы этой библиотеки позволяют считывать данные из строк и выводить данные в строки. Класс `istringstream` — это строковый поток ввода, а класс `ostringstream` — строковый поток вывода. В коде ниже стандартный класс  `istringstream` упрощает разбор строки:
...
```
Класс  `istringstream` позволяет считывать данные из содержимого строки подобно тому, как объект `cin` делает это со стандартным потоком ввода. То есть сначала считываем строку функцией `getline`, затем создаём новый строковый поток `command`. Он нужен, чтобы разобрать содержимое считанной строки.
