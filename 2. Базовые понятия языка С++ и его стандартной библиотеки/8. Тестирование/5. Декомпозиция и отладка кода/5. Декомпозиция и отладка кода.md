...
Вы научились декомпозировать и тестировать код. В этом уроке вы примените новые навыки для проверки программы «Автобусные остановки». Эта небольшая программа представляет собой простую базу данных для хранения автобусных остановок и автобусов, проходящих, через них. Программа ожидает запросы, например на ввод остановок, и выдает ответы, например выводит все остановки маршрута.

Ниже приведены условие задачи и правильное решение, целиком написанное внутри функции `main`. Ваша задача – сделать декомпозицию кода, чтобы получившаяся программа корректно решала задачу.
Условие задачи
Реализуйте систему хранения автобусных маршрутов. Обработайте следующие запросы:
- NEW_BUS bus stop_count stop1 stop2 ... — добавить маршрут автобуса с параметрами `bus`, `stop_count` и `stop1`, `stop2`, ... Эти параметры команды NEW_BUS задают название маршрута (bus), количество остановок (`stop_count`) и их названия.
Например, запрос `NEW_BUS golden_ring 4 sergiev_posad rostov ivanovo vladimir`
